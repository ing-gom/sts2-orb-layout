...
**Slay the Spire 2** 디펙트의 구체 슬롯 위치를 원하는 곡선 — 직선, 완만한 호, S자 — 어떤 모양으로든 자유롭게 재배치할 수 있는 모드입니다. 구체가 추가/소비되어 슬롯 수가 변해도 같은 곡선 위에 자동으로 다시 분포됩니다.
...
- **로컬 디펙트 플레이어에만 적용** — 시각적 변경뿐, 게임 로직에는 손대지 않음
- 매니페스트에 `"affects_gameplay": false` 표기 — 멀티플레이 중에도 안전

## 조작

전투 중 디펙트(또는 구체 슬롯이 있는 캐릭터)로 플레이할 때 사용합니다.
